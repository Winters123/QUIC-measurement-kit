...
class TPerfClient : public quic::QuicSocket::ConnectionCallback,
 ...
 public:
  ...
  void averageThroughput(){
    ...
    printf("Received %lu bytes in %lu sec\n", receivedBytes_,
      duration_.count());
    printf("Average throughput: %lf Mb/s\n", 
  ...
  void rtThroughput(){
    constexpr double bytesPerMegabit = 131072;
    uint64_t receivedBytes_bf = 0;
    uint64_t receivedBytes_af, deltaReceivedBytes;
    //update RT-throughput every second
    Timer t = Timer();
    t.setInterval([&](){
      receivedBytes_af = receivedBytes_;
      deltaReceivedBytes = receivedBytes_af - receivedBytes_bf;
      receivedBytes_bf = receivedBytes_af;
      printf("Average throughput: %lf Mb/s\n"
          , (deltaReceivedBytes / bytesPerMegabit) / 1.0);
      fprintf(f, "%lf\n"
          , (deltaReceivedBytes / bytesPerMegabit) / 1.0);
    }, 1000);
  }
...
int main(int argc, char* argv[]) {
  ...
  if (FLAGS_mode == "server") {
    ...
    //open "throughput.txt" for recording
    f = fopen("throughput.txt", "a");
    ...
    TPerfClient client(
        ...
        flagsToCongestionControlType(FLAGS_congestion),
        FLAGS_max_receive_packet_size);
    //open up a thread to print real-time throughput every second.
    //client.rtThroughput();
    client.start();
    client.averageThroughput();
    fclose(f);
  }
  return 0;
}
